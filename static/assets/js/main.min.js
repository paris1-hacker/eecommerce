...
// max and min slider that update the max and minn values whenever a page reloads...
    filterSlider: function () {
        var $slider = $("#price-slider"); // the slider
        var $priceText = $("#filter-price-range"); // display text
        var $minInput = $("#min-price"); // hidden min input
        var $maxInput = $("#max-price"); // hidden max input

        if ($slider.length) {
            noUiSlider.create($slider[0], {
                start: [1000, 100000],       // starting slider values
                connect: true,
                step: 100,
                margin: 100,
                range: { min: 0, max: 100000 } // max slider value
            });
            ...
            $slider[0].noUiSlider.on("update", function (values) {
                ...
                // Update display
                $priceText.text("₦" + min.toLocaleString() + " - ₦" + max.toLocaleString());

                // Update hidden inputs for form submission
                if ($minInput.length) $minInput.val(min);
                if ($maxInput.length) $maxInput.val(max);
            });
        }
},
...
const updatebtn = document.getElementsByClassName('add-cart')
...
function addToCart(productId, action){
  ...
  fetch(url, {
    ...
    // let qtyElement = document.getElementById(`itemqty[data-product="${productId}"]`)
    // if(qtyElement){
    //   qtyElement.innerText = data.quantity
    // }

    // // updating the subtotal
    // let subtotalElement = document.querySelector(`.subtotal-price[data-product="${productId}"]`)
    // if(subtotalElement){
...
function addCookieItem(productId, action){
  console.log('User is not Loggeed in')

  if (action == 'add'){
    // first check if our cart has that product id in it
    if(cart[productId] == undefined){ // if the iteem is not in the cart 
        cart[productId] = {
            'quantity': 1
        }
    } else{
